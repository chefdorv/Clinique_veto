...
public class PersonnelDAOjdbcImpl implements PersonnelsDAO {
    ...
    public Personnel read(int id) throws DALException {
        ...
        try {
            ...
            if (rs.next()) {
                try {
                    perso = new Personnel(rs.getInt("Code"), rs.getString("Nom"),rs.getString("Prenom"), rs.getString("Mot de passe"), rs.getString("Role"), rs.getInt("Sexe"), rs.getBoolean("Archive"));
                } catch (SQLException e) {
            ...
            while (rs.next()) {
                try {
                    perso = new Personnel(rs.getInt("CodePers"), rs.getString("Nom"), rs.getString("Prenom"),
                            rs.getString("MotPasse"), rs.getString("Role"), rs.getInt("Sexe"), rs.getBoolean("Archive"));
                    lstPerso.add(perso);
                } catch (SQLException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
